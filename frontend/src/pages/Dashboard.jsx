...
import { STATUS_LABELS, CATEGORY_LABELS } from '../constants.js'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadItems() {
      try {
        const { data } = await api.get('/media-items', { params: { size: 50, sort: 'updatedAt,desc' } })
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="rounded-2xl border border-ink-700 bg-ink-900">
          <div className="px-5 py-4 border-b border-ink-800 flex items-center justify-between">
            <h2 className="font-display text-lg text-stone-50">Recent activity</h2>
          </div>
